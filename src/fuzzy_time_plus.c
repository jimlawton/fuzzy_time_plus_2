 ...
 *
 * Background inverts depending on day/night.
 */
...
void updateLayer(TextLine *animating_line, int line) {
    ...
    animation_set_handlers(&animating_line->layer_animation[0].animation, (AnimationHandlers ) { .stopped =
                                    (AnimationStoppedHandler) animationInStoppedHandler },
                           (void *) outside);
...
void pbl_main(void *params) {
    PebbleAppHandlers handlers = {
        ...
        // Handle time updates
        .tick_info = { .tick_handler = &handle_minute_tick, .tick_units = MINUTE_UNIT }
    };
